...
class BidCardExtractor:
    ...
    def search_bid(self, driver, search_keyword):
        try:
            ...
            self._print_and_write(f"An error occurred during the search: {e}", driver)
    ...
    def extract_record_summary(self, driver):
        try:
            ...
            if match:
                ...
                self._print_and_write("Could not extract the total number of records.", driver)
                return None
        except Exception as e:
            self._print_and_write(f"An error occurred while extracting the record summary: {e}", driver)
            return None

    def open_output_file(self):
        self.file = open(self.output_file, 'w', encoding='utf-8')

    def _print_and_write(self, text, driver=None):
        print(text)
    ...
    def process_page(self, page_num, search_keyword, output_queue):
        ...
        try:
            self.open_webpage(driver, self.url)
            self.search_bid(driver, search_keyword)

            if not self.record_summary_printed:
                self.extract_record_summary(driver)
            ...
            if cards:
                page_output = [f"Page {page_num}:"]
                for card in cards:
                    current_card_number = page_num * len(cards) + cards.index(card) + 1
                    card_output = self.process_card(card, current_card_number)
                    if card_output:
                        page_output.append(card_output)
    ...
    def extract_and_print_cards(self):
        ...
        while True:
            try:
                ...
                processes.append(multiprocessing.Process(target=self.process_page, args=(page_num, search_keyword, output_queue)))
                processes[-1].start()
                page_num += 1
                ...
                while not output_queue.empty():
                    self._print_and_write(output_queue.get())
...
if __name__ == "__main__":
    ...
    output_file = "output/searching_demo.txt"  # Path to the output file
